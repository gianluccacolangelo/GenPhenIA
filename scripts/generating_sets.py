...
result_2_total_obs = dict(sorted(Counter([y for x in result_2[1] for y in
    x]).items()))
bitgenia_total_obs = dict(sorted(Counter([y for x in bitgenia_results[1] for y
    in x]).items()))

with plt.style.context(['science','ieee','nature']):
    fig,ax = plt.subplots()
    ax.bar(result_total_obs.keys(),result_total_obs.values(),label='simulados',alpha=0.5)
    ax.bar(result_2_total_obs.keys(),result_2_total_obs.values(),label='simulados 2',alpha=0.5)
    ax.bar(bitgenia_total_obs.keys(),bitgenia_total_obs.values(),label='bitgenia',alpha=0.5)
...
with plt.style.context(['science','ieee','nature']):
    ...
    ax.set_title('Similaridad entre los \nfenotipos reales y sintéticos\npara el mismo gen',fontsize=6)
    ax.set_xlim(0,1)
## }}}
...
exact_clinv_values, exact_clinv_counts=(np.array([0.        , 0.05555556,
    0.11111111, 0.125, 0.13043478,0.13888889, 0.14285714, 0.16666667,
    0.18518519, 0.2,0.21428571, 0.22222222, 0.25      , 0.27777778,
    0.28571429,0.3       , 0.30769231, 0.3125    , 0.33333333,
    0.35294118,0.35714286, 0.36363636, 0.375     , 0.38461538, 0.38888889,0.4
    , 0.40740741, 0.4137931 , 0.41666667, 0.42857143,0.44      , 0.44444444,
    0.45833333, 0.46153846, 0.5       ,0.51515152, 0.52631579, 0.52941176,
    0.53333333, 0.53846154,0.54545455, 0.55555556, 0.57142857, 0.58333333, 0.6
    ,0.60869565, 0.61111111, 0.61538462, 0.625, 0.63636364,0.64285714,
    0.64705882, 0.66666667, 0.69230769, 0.7       ,0.7037037 , 0.70588235,
    0.71428571, 0.72222222, 0.72727273,0.73913043, 0.75      , 0.77777778,
    0.7826087 , 0.8       ,0.82352941, 0.83333333, 0.84615385, 0.85714286,
    0.88888889,0.9       , 0.90909091, 0.92307692, 0.94117647, 1.
    ]),np.array([434,   1,   1,   1,   1,   1,   2,  19,1,   6,   1,   2,   7, 1,   6,   1,   3,   2,  37,   1,   1,4,   1,   2,   1,   9,1,   1,   3,   4,   1,   4,   1,   1,  84,   1,   1,   1,   2,2,   7,   5,   7,   2,  12,   1,   1,1,   4,   3,   1,   1,23,   3,   2,   1,   1,   1,   1,   2,1,  12,   1,   1,   6,1,   6,   1,   2,   1,   1,   1,   1,1, 109]))


inexact_clinv_values,inexact_clinv_counts = (np.array([0.        , 0.05882353, 0.0625    , 0.06666667, 0.09090909,0.14285714, 0.15789474, 0.16666667, 0.18181818, 0.2       ,0.25      , 0.3       , 0.33333333, 0.36363636, 0.4       ,0.42857143, 0.5       , 0.66666667, 0.75, 0.8       ,1.        ]),
...
hist_counts, _ = np.histogram(inexact_clinv_values, bins=bins, weights=inexact_clinv_counts)

with plt.style.context(['science','ieee','nature']):
    fig,ax = plt.subplots()
    ax.bar(bins[:-1], hist_counts, width=(bins[1] - bins[0]),
            align='edge',alpha=0.7)
    ax.set_xlabel('Proporción',fontsize=6)
    ax.set_ylabel('Frecuencia')
    ax.set_title('Distribución de los fenotipos\nvagos para los inexactos\nregistrados de ClinVar',fontsize=6)
    ax.set_xlim(0,1)
## }}}
