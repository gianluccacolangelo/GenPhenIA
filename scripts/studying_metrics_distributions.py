...
def calculate_gene_parameters(
    mph, iph, type_of_noise, alpha, betha, gamma, gen_sample, fen_sample
):
    """
...
especificidad_n = np.array(
    [
        distributions("similaridad", 0.5, 0.5, "constant", fen_sample, 500, 1, 1, 1)
...
with plt.style.context(["science", "ieee", "nature"]):
    ...
    plt.subplots_adjust(
        top=0.9, bottom=0.15, left=0.176, right=0.952, hspace=0.0, wspace=0.1
    )
