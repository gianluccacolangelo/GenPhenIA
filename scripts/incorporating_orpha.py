...
phen_properties_dict = {
    row["Unnamed: 0"]: (row["num_children"], row["num_ancestors"])
    for index, row in phen_properties.iterrows()
}
## }}}
...
# Load the file
genemap2_df = pd.read_csv(
    f"{PATH}data/OMIM/genemap2.txt",
    comment="#",
    delimiter="\t",
    names=columns,
    skiprows=4,
...
gene_to_entrez = pd.Series(
    genemap2_df["Entrez Gene ID"].values, index=genemap2_df["Approved Gene Symbol"]
).to_dict()
entrez_to_gene = pd.Series(
    genemap2_df["Gene Symbols"].values, index=genemap2_df["Entrez Gene ID"]
).to_dict()
    ...
    for disorder_set_status in root.find("HPODisorderSetStatusList"):
        ...
        if disorder is not None:
            ...
            if disorder_name == True:
                ...
                for hpo_disorder_association in disorder.find(
...
def parse_xml_to_gene_dict(root, disorder_name=False):
    ...
    for disorder in root.find("DisorderList"):
        ...
        if disorder_name == True:
            ...
            for disorder_gene_association in disorder.find(
...
def rank_diseases(observed_phenotypes, phen_disease_dict, threshold=3):
        ...
        for observed_phenotype in observed_phenotypes:
            ...
            if observed_phenotype in phenotypes:
                ...
                if closest_term is not None:
                    ...
                    total_weight += empirical_weight - empirical_weight * (
                        closest_distance / branch_size
                    )
                    count += 1
    ...
    ranked_diseases = sorted(
        total_weights.items(), key=lambda item: item[1], reverse=True
    )
...
def v2_model_evaluation(clinical_cases, top_diseases=50, top_genes=4000):
    ...
    for observed_genes in clinical_cases.keys():
        ...
        ranked_diseases = rank_diseases(
            observed_phenotypes, phen_disease_dict, threshold=1
        )
        ...
        if observed_genes in candidate_genes:
            ...
            for observed_phenotype in observed_phenotypes:
                ...
                closest_term = find_closest_term(observed_phenotype, gold_stand_phens)[
                    0
                ][1]
                closest_distance = find_closest_term(
                    observed_phenotype, gold_stand_phens
                )[1]
        ...
        try:
            print(
                f"ranking: {candidate_genes.index(observed_genes)}, distances: {distances}"
            )
        except:
    ...
    while queue:
        ...
        if term not in seen:
            ...
            queue.extend(
                ...
                for neighbour in term_relationships.get(term, [])
            )

    return (source, closest_term, distance) if closest_term else (source, None, None)


closest = find_closest_term(
    "HP:0000001", ["HP:0000005", "HP:0000006"], term_relationships=term_relationships
)
## }}}
