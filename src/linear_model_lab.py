...
def calculate_gene_parameters(
    set_of_phens, alpha, beta, gamma, n_metrica, nueva_metrica, precision=True
):
    """
        ...
        data.append(
            {
                ...
                "total": (
                    alpha * especificidad + beta * capitalidad + gamma * similaridad
                ),
            }
        )
...
def model_evaluating(
    ...
    for gene in list_of_genes:
        ...
        fen_observados = pgw.fen_observados_con_ruido(gene, noised_set, fen_sample)

        # Calculamos los parámetros esp. cap. y sim. para la unión de genes
        # posibles que causan esos fenotipos

        df = calculate_gene_parameters(
            fen_observados,
            alpha,
            beta,
            gamma,
...
def model_real_evaluating(
    db_real, alpha, beta, gamma, nueva_metrica, n_metrica, precision
):
    """
